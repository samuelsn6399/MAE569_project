...
jupiter_dDate1 = datetime(2050,06,20,11,58,00);
earth_aDate2 = jupiter_dDate1+740;
...
r_soie = R_soie.*(-v2_unit); % [km]

[a_flyby,e_flyby,i_flyby,O_flyby,o_flyby,nu_flyby] = orbitalElements(r_soie,v_infea1_ijk,mu_e)
[dumby,dumby,R_per2equ] = perifocal2Equatorial([0;0;0],[0;0;0],i_flyby,O_flyby,O_flyby)


v_infea1_pkw = R_per2equ\v_infea1_ijk';
v_infea1_pkw_mag = sqrt(dot(v_infea1_pkw,v_infea1_pkw))
v_infed1_pkw = infa2infd*v_infea1_pkw; % [km/s]
v_infed1_pkw_mag = sqrt(dot(v_infed1_pkw,v_infed1_pkw))
v_infed1_ijk = R_per2equ*v_infed1_pkw; % [km/s]

% v_infed1 = infa2infd*v_infea1'; % [km/s]

v_infed1_mag = sqrt(dot(v_infed1_ijk,v_infed1_ijk)); % [km/s]
v_infed1AU = v_infed1_ijk'.*(1/AUTU2kms); % [AU/TU]
...
r_satTraj34_complete = zeros(length(T_deepspaceTraj_dayidx),3);
for idx = 1:length(T_deepspaceTraj_dayidx)
    ToFidx = getTU(idx,'d');
...
r_satTraj45_complete = zeros(length(T_deepspaceMan_dayidx),3);
for idx = 1:length(T_deepspaceMan_dayidx)
    ToFidx = getTU(idx,'d');
    r_satTraj45_complete(idx,:) = keplarToF(a_s4, e_s4, i_s4, O_s4, o_s4,r4,v4,theta_s4,ToFidx);
end
plot3(r_satTraj12(:,1),r_satTraj12(:,2),r_satTraj12(:,3),'Color',[0 1 0],'LineStyle','--','LineWidth',1,'DisplayName','Sat Trajectory 1')
plot3(r_satTraj34(:,1),r_satTraj34(:,2),r_satTraj34(:,3),'Color',[0 1 0],'LineStyle',':','LineWidth',2,'DisplayName','Sat Trajectory 2')
plot3(r_satTraj45(:,1),r_satTraj45(:,2),r_satTraj45(:,3),'Color',[0 1 0],'LineStyle','-.','LineWidth',1,'DisplayName','Sat Trajectory 3')
...
deltaV2_min
deltaV3_min
deltaV = deltaV1+deltaV2_min+deltaV3_min